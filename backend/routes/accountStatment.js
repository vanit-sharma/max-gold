...
// Helpers: parse inputs in Europe/London and produce inclusive start / exclusive end
function parseStartLondon(str) {
  // Accepts "MM/DD/YYYY hh:mm A" or "YYYY-MM-DD"
  const m = moment.tz(
    str,
    ["YYYY-MM-DD hh:mm A", "YYYY-MM-DD"],
    true,
    LONDON_TZ
  );
  return m.isValid()
    ? (str.includes(":") ? m.clone() : m.startOf("day")).toDate()
    : null;
}

function parseEndExclusiveLondon(str) {
  // If date-only -> next day start (exclusive end). If time provided -> use exact time as exclusive end.
  const m = moment.tz(
    str,
    ["YYYY-MM-DD hh:mm A", "YYYY-MM-DD"],
    true,
  ...
  const endExclusive = str.includes(":")
    ? m
    : m.endOf("day").add(1, "millisecond");
  // Better: make end exclusive the next millisecond (or simply next day start for date-only)
  return endExclusive.toDate();
}
