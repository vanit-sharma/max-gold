...
router.post("/profit-loss-by-user", auth, async (req, res) => {
  ...
  returnObj.fancy = fancy * -1;
  returnObj.soccer = Math.round(football_set + football) * -1;
  returnObj.tennis = Math.round(tennis) * -1;
  returnObj.greyhound = Math.round(greyhound) * -1;
  returnObj.horseRace = Math.round(horseRace) * -1;
  returnObj.tpStudio = Math.round(tpStudio) * -1;
  returnObj.galexyStudio = Math.round(galexyStudio) * -1;
  returnObj.betfairGames = Math.round(betfairGames) * -1;
  returnObj.worldCasino = Math.round(worldCasino) * -1;
  grandTotal =
    returnObj.cricket +
    fancy +
    football_set +
    ...
    galexyStudio +
    betfairGames;
  returnObj.grandTotal = Math.round(grandTotal) * -1;
...
router.post("/pl-details", auth, async (req, res) => {
  ...
  for (let i = 0; i < recordList.length; i++) {
    ...
    arr.amount = Math.round(objValue.amount) * -1;
