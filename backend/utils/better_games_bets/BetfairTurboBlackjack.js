...
async function placeBet(req, res) {
  ...
  if (
    ...
    const qds = await BzBetfairTurboBlackjackRate.aggregate([
      {
        $match: {
          evt_status: "ACTIVE",
          cat_mid: catmid,
    ...
    if (userHistoryQuery.length) {
      ...
      if (ratok == 1) {
        pointok = 0;
        const drstc = await BzBetfairTurboBlackjackMatch.findOne({
          mid_mid: catmid,
        ...
        if (drstc) {
          ...
          if (userHistoryQuery[0].bz_balance - cla >= 0) {
            let stm = await BzBetfairTurboBlackjackMatch.updateOne(
              { mid_mid: catmid, 
                //user_id: req.user._id, 
                user_id: req.user._id.toHexString(),
                rnr_nam: teamname },
          ...
          if (lib <= userHistoryQuery[0].bz_balance) {
            drsts = await BzBetfairTurboBlackjackRate.find({
              cat_mid: catmid,
            }).lean();
            ...
            if (userHistoryQuery[0].bz_balance - cla >= 0) {
              ...
              const stm = await BzBetfairTurboBlackjackMatch.create({
                mid_mid: catmid,
        ...
        if (pointok == 1) {
          ///
          await BzBetfairTurboBlackjackRate.updateOne(
            { evt_id: catmid },
            { $set: { is_bet_place: "1" } }
          );
          ///

          const betHistory = await BzBetfairTurboBlackjackBetHistory.create({
            uname: req.user.uname,
            user_id: req.user._id,
            cat_mid: catmid,
            rnr: rnr,
          ...
          if (st2) {
            ...
            await LiveNotifications.create({
              ...
              user_id: req.user._id,
              game_type: "6",
              description: `${req.user._id} placed bet on ${teamname} in Live Betfair Turbo Blackjack casino games.`,
            });
