...
function validateCanPlaceBet(req, res) {
  ...
  const { amount, catmid, teamname, bettype, odds, selectRnr, round, marketVal } =
    req.body;
...
async function placeBet(req, res) {
  ...
  if (
    ...
    if (teamname == "Hand 1") {
      ...
      sid = catmid + "-4";
    } else if (teamname == "Winner has FH or better") {
      if (bettype == "b") {
        if (selectRnr == 1) {
          rnr = "runner5b1";
        } else if (selectRnr == 2) {
      ...
      sid = catmid + "-5";
    } else if (teamname == "Winner has Flush") {
      if (bettype == "b") {
        if (selectRnr == 1) {
          rnr = "runner6b1";
        } else if (selectRnr == 2) {
      ...
      sid = catmid + "-6";
    } else if (teamname == "Winner has Straight") {
      if (bettype == "b") {
        if (selectRnr == 1) {
          rnr = "runner7b1";
        } else if (selectRnr == 2) {
      ...
      sid = catmid + "-7";
    } else if (teamname == "Winner has Three of a Kind") {
      if (bettype == "b") {
        if (selectRnr == 1) {
          rnr = "runner8b1";
        } else if (selectRnr == 2) {
      ...
      sid = catmid + "-8";
    } else if (teamname == "Winner has Two Pair or worse") {
      if (bettype == "b") {
        if (selectRnr == 1) {
          rnr = "runner9b1";
        } else if (selectRnr == 2) {
    ...
    const qds = await BzBetfairOmahaRate.aggregate([
      {
        $match: {
          evt_status: "ACTIVE",
          cat_mid: catmid,
    ...
    if (userHistoryQuery.length) {
      ...
      let return_array = {};
      return_array.game_name = "BetfairOmaha";
      ...
      if (
        ...
        let lcat_rnr9_status =
          ...
          console.log('st->',lcat_rnr5_status);
      ...
      if (bettype.toLowerCase() == "b") {
        ...
        if (odds >= liverate && liverate > 0 && !empty(liverate)) {
          ratok = 1;
          ///
          rat = liverate;
          ///
      ...
      if (ratok == 1) {
        pointok = 0;
        const drstc = await BzBetfairOmahaMatch.findOne({
          cat_mid: catmid,
        ...
        if (drstc) {
          ...
          limite = drstc.lockamt + drstc.rnr5s + userHistoryQuery[0].bz_balance;
          console.log('marketVal->',marketVal)
          if (marketVal == 1) {
            if (
              rnr == "runner1b1" ||
            ...
            if (
              ...
              limit = min(limitb, limitc, limitd, limite);
            }
            if (
              rnr == "runner6b1" ||
              rnr == "runner6b2" ||
              ...
              limit = min(limita, limitc, limitd, limite);
            }
            if (
              rnr == "runner7b1" ||
              rnr == "runner7b2" ||
              ...
              limit = min(limitb, limita, limitd, limite);
            }
            if (
              rnr == "runner8b1" ||
              rnr == "runner8b2" ||
              ...
              limit = min(limitb, limitc, limita, limite);
            }
            if (
              rnr == "runner9b1" ||
              rnr == "runner9b2" ||
              ...
              limit = min(limitb, limitc, limita, limitd);
            }
          ...
          console.log('lib',lib,'--limit',limit);
          if (lib <= limit) {
            ...
            if (Math.min(nla1, nla2, nla3, nla4, nla5) < 0)
              nla = Math.abs(Math.min(nla1, nla2, nla3, nla4, nla5));
            else nla = 0;
            ...
            if (userHistoryQuery[0].bz_balance - cla >= 0) {
              let stm = await BzBetfairOmahaMatch.updateOne(
                {
                  cat_mid: catmid,
                  user_id: req.user._id,
                  market_type: marketVal,
          ...
          if (lib <= userHistoryQuery[0].bz_balance) {
            drsts = await BzBetfairOmahaRate.find({
              cat_mid: catmid,
            }).lean();
            ...
            if (userHistoryQuery[0].bz_balance - cla >= 0) {
              ...
              if (marketVal == 1) {
                ...
                sidVal5 = drsts.cat_sid9;

                name1 = drsts.cat_rnr5;
                name2 = drsts.cat_rnr6;
                name3 = drsts.cat_rnr7;
                name4 = drsts.cat_rnr8;
                name5 = drsts.cat_rnr9;
              }

              const stm = await BzBetfairOmahaMatch.create({
                cat_mid: catmid,
        ...
        if (pointok == 1) {
          ///
          await BzBetfairOmahaRate.updateOne(
            { evt_id: catmid },
            { $set: { is_bet_place: "1" } }
          );
          ///

          const betHistory = await BetfairOmahaBetHistory.create({
            uname: req.user.uname,
            user_id: req.user._id,
            cat_mid: catmid,
            rnr: rnr,
          ...
          await pushExposure(req, res, sid, cla, "Live Betfair Omaha");
