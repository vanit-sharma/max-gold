...
async function placeBet(req, res) {
  ...
  if (
    ...
    const qds = await BzBetfairHoldRate.aggregate([
      {
        $match: {
          evt_status: "ACTIVE",
          cat_mid: catmid,
    ...
    if (userHistoryQuery.length) {
      ...
      let return_array = {};
      return_array.game_name = "BetfairHold";
      return_array.result = false;
      let gameId = "1444074";
      let jsonds = await getBetfairGamesData(gameId);
      if(jsonds == 'ER101'){
        console.log('Without VPN BEtfair not working',jsonds);
        return res.status(400).json({
          status: false,
      ...
      if (ratok == 1) {
        pointok = 0;
        const drstc = await BzBetfairHoldMatch.findOne({
          cat_mid: catmid,
...
/*
              let stm = await BzBetfairHoldMatch.updateOne(
...
*/
          ...
          if (lib <= userHistoryQuery[0].bz_balance) {
            drsts = await BzBetfairHoldRate.find({
              cat_mid: catmid,
            }).lean();
            ...
            if (userHistoryQuery[0].bz_balance - cla >= 0) {
              ...
              console.log("before insertion in BzBetfairHoldMatch.create: ", {
                cat_mid: catmid,
              ...
              const stm = await BzBetfairHoldMatch.create({
                cat_mid: catmid,
                uname: req.user.uname,
                user_id: req.user._id,
                rnr1: drsts[0].cat_rnr1,
        ...
        if (pointok == 1) {
          ///
          await BzBetfairHoldRate.updateOne(
            { evt_id: catmid },
            { $set: { is_bet_place: "1" } }
          );
          ///
          ...
          const betHistory = await BzBetfairHoldBetHistory.create({
            uname: req.user.uname,
            user_id: req.user._id,
            cat_mid: catmid,
            rnr: rnr,
          ...
          // Add exposure here
          await pushExposure(req, res, sid, cla, "Live Betfair Hold");
          if (st2) {
            balance_point =
