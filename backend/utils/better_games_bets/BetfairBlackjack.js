...
async function placeBet(req, res) {
  ...
  if (
    ...
    if (teamname == "Player 1") {
      ...
      sid = catmid + "-4";
      rnr_val = 4;
    } else if (teamname == "Dealer wins or ties all") {
      if (bettype == "b") {
        if (selectRnr == 1) {
          rnr = "runner5b1";
        } else if (selectRnr == 2) {
      ...
      sid = catmid + "-5";
      rnr_val = 5;
    } else if (teamname == "One or more has 5 cards") {
      if (bettype == "b") {
        if (selectRnr == 1) {
          rnr = "runner6b1";
        } else if (selectRnr == 2) {
      ...
      sid = catmid + "-6";
      rnr_val = 6;
    } else if (teamname == "All cards unique") {
      if (bettype == "b") {
        if (selectRnr == 1) {
          rnr = "runner7b1";
        } else if (selectRnr == 2) {
      ...
      sid = catmid + "-7";
      rnr_val = 7;
    } else if (teamname == "Only one has 21pts") {
      if (bettype == "b") {
        if (selectRnr == 1) {
          rnr = "runner8b1";
        } else if (selectRnr == 2) {
    ...
    const qds = await BzBetfairBlackjackRate.aggregate([
      {
        $match: {
          evt_status: "ACTIVE",
          cat_mid: catmid,
    ...
    console.log('qds->',qds[0]);
    console.log('result->',qds[0].result);
    if (qds.length) {
      stld = qds[0].stld;
    ...
    let isBettingEnable = ds ? ds.betfair_blackjack : false;
    ...
    if (!isBettingEnable) {
      return res
        ...
        .json({
          status: false,
          message: "Betting not open for Betfair Blackjack.",
        });
    }
    ...
    console.log('match_status->',match_status,'stld->',stld,'roundResult->',roundResult);
    if (
    ...
    if (userHistoryQuery.length) {
      ...
      let return_array = {};
      return_array.game_name = "BetfairBlackjack";
      ...
      if (
        ...
        let lcat_rnr5_status =
          obj.game.markets.market.selections.selection[4].status;
        let lcat_rnr6_status =
          obj.game.markets.market.selections.selection[5].status;
        let lcat_rnr7_status =
          obj.game.markets.market.selections.selection[6].status;
        let lcat_rnr8_status =
          obj.game.markets.market.selections.selection[7].status;
        ...
        if (String(lcat_mid).trim() == String(evt_id).trim()) {
          if (lcat_sid1 == sid && lcat_rnr1_status == "IN_PLAY") {
            ...
            if (bettype === "b") {
              liverate =
                obj.game.markets.market.selections.selection[4]
                  .bestAvailableToBackPrices.price[0]._;
            } else if (bettype === "l") {
              liverate =
                obj.game.markets.market.selections.selection[4]
            ...
            if (bettype === "b") {
              liverate =
                obj.game.markets.market.selections.selection[5]
                  .bestAvailableToBackPrices.price[0]._;
            } else if (bettype === "l") {
              liverate =
                obj.game.markets.market.selections.selection[5]
            ...
            if (bettype === "b") {
              liverate =
                obj.game.markets.market.selections.selection[6]
                  .bestAvailableToBackPrices.price[0]._;
            } else if (bettype === "l") {
              liverate =
                obj.game.markets.market.selections.selection[6]
            ...
            if (bettype === "b") {
              liverate =
                obj.game.markets.market.selections.selection[7]
                  .bestAvailableToBackPrices.price[0]._;
            } else if (bettype === "l") {
              liverate =
                obj.game.markets.market.selections.selection[7]
      ...
      console.log("liverate->", liverate, "odds->", odds);
      ...
      if (ratok == 1) {
        pointok = 0;
        const drstc = await BzBetfairBlackjackMatch.findOne({
          mid_mid: catmid,
        ...
        if (drstc) {
          ...
          if (userHistoryQuery[0].bz_balance - cla >= 0) {
            let stm = await BzBetfairBlackjackMatch.updateOne(
              { mid_mid: catmid,
                //user_id: req.user._id, 
                user_id:req.user._id.toHexString(),
                rnr_nam: teamname },
          ...
          if (lib <= userHistoryQuery[0].bz_balance) {
            drsts = await BzBetfairBlackjackRate.find({
              cat_mid: catmid
            }).lean();
            ...
            if (userHistoryQuery[0].bz_balance - cla >= 0) {
              ...
              const stm = await BzBetfairBlackjackMatch.create({
                mid_mid: catmid,
        ...
        if (pointok == 1) {
          ///
          await BzBetfairBlackjackRate.updateOne(
            { evt_id: catmid },
            { $set: { is_bet_place: "1" } }
          );
          ///

          const betHistory = await BzBetfairBlackjackBetHistory.create({
            uname: req.user.uname,
            user_id: req.user._id,
            cat_mid: catmid,
            rnr: rnr,
          ...
          await pushExposure(req, res, sid, cla, "Live Betfair Blackjack");
          ...
          if (st2) {
            ...
            await UserLogs.create({
              page: "livebet_betfair_blackjack",
              linkid: bet_id,
              ptrans: cla,
              otrans: "",
              points: claVal,
            ...
            await LiveNotifications.create({
              evt_id: "Live Betfair Blackjack",
              user_id: req.user._id,
              game_type: "6",
              description: `${req.user._id} placed bet on ${teamname} in Live Betfair Blackjack casino games.`
            });
