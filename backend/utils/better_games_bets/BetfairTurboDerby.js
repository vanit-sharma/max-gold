...
const BzBetfairTurboDerbyRate = require("../../models/BzBetfairTurboDerbyRate");
const BzBetfairTurboDerbyMatch = require("../../models/BzBetfairTurboDerbyMatch.js");
...
async function placeBet(req, res) {
  ...
  if (
    ...
    const qds = await BzBetfairTurboDerbyRate.aggregate([
      {
        $match: {
          evt_status: "ACTIVE",
          cat_mid: catmid,
    ...
    if (userHistoryQuery.length) {
      ...
      if (ratok == 1) {
        pointok = 0;
        const drstc = await BzBetfairTurboDerbyMatch.findOne({
          cat_mid: catmid,
        ...
        if (drstc) {
          ...
          if (lib <= limit) {
            ...
            if (userHistoryQuery[0].bz_balance - cla >= 0) {
              let stm = await BzBetfairTurboDerbyMatch.updateOne(
                {
                  cat_mid: catmid,
                  user_id: req.user._id,
                  market_type: marketVal,
          ...
          if (lib <= userHistoryQuery[0].bz_balance) {
            drsts = await BzBetfairTurboDerbyRate.find({
              cat_mid: catmid,
            }).lean();
...
console.log('+++++++++++++++++ line 802');
              const stm = await BzBetfairTurboDerbyMatch.create({
                cat_mid: catmid,
        ...
        if (pointok == 1) {
          ...
          await BzBetfairTurboDerbyRate.updateOne(
            { evt_id: catmid },
            { $set: { is_bet_place: "1" } }
          );
          ///

          const betHistory = await BetfairTurboDerbyBetHistory.create({
            uname: req.user.uname,
            user_id: req.user._id,
            cat_mid: catmid,
            rnr: rnr,
