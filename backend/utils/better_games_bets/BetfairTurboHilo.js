...
async function placeBet(req, res) {
  ...
  if (
    ...
    const qds = await BzBetfairTurboHiloRate.aggregate([
      {
        $match: {
          evt_status: "ACTIVE",
          cat_mid: catmid,
    ...
    if (userHistoryQuery.length) {
      ...
      if (ratok == 1) {
        pointok = 0;
        const drstc = await BzBetfairTurboHiloMatch.findOne({
          mid_mid: catmid,
        ...
        if (drstc) {
          ...
          if (userHistoryQuery[0]["bz_balance"] - cla >= 0) {
              let stm = await BzBetfairTurboHiloMatch.updateOne(
          ...
          if (lib <= userHistoryQuery[0].bz_balance) {
            drsts = await BzBetfairTurboHiloRate.find({
              cat_mid: catmid,
            }).lean();
            ...
            if (userHistoryQuery[0].bz_balance - cla >= 0) {
              ...
              const stm = await BzBetfairTurboHiloMatch.create({
                mid_mid: catmid,
        ...
        if (pointok == 1) {
          ///
          await BzBetfairTurboHiloRate.updateOne(
            { evt_id: catmid },
            { $set: { is_bet_place: "1" } }
          );
          ///

          const betHistory = await BzBetfairTurboHiloBetHistory.create({
            uname: req.user.uname,
            user_id: req.user._id,
            cat_mid: catmid,
            rnr: rnr,
          ...
          await pushExposure(req, res, sid, cla, "Live Betfair Turbo Hilo");
          ...
          if (st2) {
            ...
            await UserLogs.create({
              page: "livebet_betfair_turbo_hilo",
              linkid: bet_id,
              ptrans: cla,
              otrans: "",
              points: claVal,
