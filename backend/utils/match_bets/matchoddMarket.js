...
const validatePlaceLiveBets = [
  body("catmid")
    .trim()
    .notEmpty()
    .withMessage("Mid is required")
    .isLength({ max: 20 })
    .withMessage("Mid not Valid"),
...
const placeLiveBets = async (req, res) => {
  ...
  try {
    const { bet_type, odds, amount, catmid, sid: cat_sid, delay } = req.body;
    let rnr = "";
    let limit = 0;
    ...
    if (userHistoryQuery && evt_id != "") {
      ...
      if (
        ...
        jsonds = await getBetfairData(catmid);
          ...
          if (t0 == "OPEN") {
            for (k = 0; k < jsonds[0].result[0].runners.length; k++) {
              if (jsonds[0].result[0].runners[k].selectionId == cat_sid) {
                ...
                } else if (bet_type.toUpperCase() == "L") {
                  liverate =
                ...
                if (
                  ...
                  if (
                    ...
                    b1Size =
                      jsonds[0].result[0].runners[k].ex.availableToBack[0]
                        ?.size;
                  }
                ...
                if (
                  ...
                  if (
                    ...
                    b2Size =
                      jsonds[0].result[0].runners[k].ex.availableToBack[1]
                        ?.size;
                  }
                ...
                if (
                  ...
                  if (
                    ...
                    b3Size =
                      jsonds[0].result[0].runners[k].ex.availableToBack[2]
                        ?.size;
                  }
        ...
        if (ratok == 1) {
          ...
          if (pointok == 1) {
            ...
            let expBetObj = {};
            expBetObj.team1_book = team1_book;
            expBetObj.team2_book = team2_book;
            expBetObj.team3_book = team3_book;

            await pushExposure(req, res, catmid, cla, event_name, expBetObj);
