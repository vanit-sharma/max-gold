...
const placeLiveBets = async (req, res) => {
  await Promise.all(
    validatePlaceLiveBets.map((validation) => validation.run(req))
  );
  ...
  if (market_type_post == "ff") {
    market_type = "footballfancy";
    market_type_value = 6;
  }
  ...
  if (betfairEvents) {
    ...
    //console.log("parent_cat_mid->", parent_cat_mid);
    const betfairEventsParent = await BetfairEvent.findOne({
      cat_mid: parent_cat_mid
    }).lean();

    eventName = betfairEventsParent.evt_evt;
    ...
    if (
        ...
        if (t0 == "OPEN") {
          for (k = 0; k < jsonds[0].result[0].runners.length; k++) {
            if (jsonds[0].result[0].runners[k].selectionId == cat_sid) {
              ...
              if (jsonds[0].result[0].runners[k].ex.availableToBack[0]?.price) {
                b1Rate =
                  jsonds[0].result[0].runners[k].ex.availableToBack[0]?.price;
              ...
              if (jsonds[0].result[0].runners[k].ex.availableToBack[1]?.price) {
                b2Rate =
              ...
              if (jsonds[0].result[0].runners[k].ex.availableToBack[2]?.price) {
                b3Rate =
              ...
              if (jsonds[0].result[0].runners[k].ex.availableToLay[0]?.price) {
                ...
                l1Size = 0;
                if (jsonds[0].result[0].runners[k].ex.availableToLay[0]?.size) {
                  l1Size =
              ...
              if (jsonds[0].result[0].runners[k].ex.availableToLay[1]?.price) {
                ...
                l2Size = 0;
                if (jsonds[0].result[0].runners[k].ex.availableToLay[1]?.size) {
                  l2Size =
              ...
              if (jsonds[0].result[0].runners[k].ex.availableToLay[2]?.price) {
                ...
                l3Size = 0;
                if (jsonds[0].result[0].runners[k].ex.availableToLay[2]?.size) {
                  l3Size =
      ...
      if (ratok == 1) {
          ...
          if (drstc) {
            ...
            if (lib <= limit) {
              ...
              if (userHistoryQuery[0].bz_balance - cla >= 0) {
                ...
                userBook.game_type = bet_game_type;
        ...
        if (pointok == 1) {
          ...
          bet_market_type = market_type_value;
