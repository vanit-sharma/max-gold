...
const liveBetFancy = async (req, res) => {
  ...
  let maxWinning = currency === 1 || currency === 2 ? 1_000_000 : 28_000;
  let b_nam = "";
  ...
  if (
    ...
    const rte = "s";
    const price = await bzFormValue(req.body.price);
    const r2 = 0;
    let dr;
    ...
    const { fancy_type, cat_mid, fancy, inplay, diamond_evt_id, evt_evt } = ev;
    ...
    if (!((inplay === 0 || inplay === 1) && fancy === 1)) {
      return res.json({
        status: false,
        message: "Betting not open for this match.2"
      });
    }
    ...
    if (!fancyDoc) {
      return res.json({ status: false, message: "Fancy not found." });
    }
    ...
    if (!rnrRows || rnrRows.length <= 0) {
      return res.json({ status: false, message: "Fancy not found11." });
    }
    drsts = rnrRows[0];
    ...
    if (userHistoryQuery && evt_id != "") {
      ...
      master_sharing = userHistoryQuery[0].master_sharing;
      ...
      if (
       ...
       //fancy_type
          switch (Number(2)) {
            case 1:
              // fancy_live_api_virtual
              jsonds2 = await fancyLiveApiVirtual(id);
              break;

            case 2:
              jsonds2 = await fancyLiveApi(diamond_evt_id, "http://www.livedataapi.net/sports_api/api/v1/cricket/fancy");
              break;

            case 3:
              jsonds2 = await sportsApingRequestProxy(id, "http://www.gigblitz.net/sn_script/proxy_fancy.php");
              break;

            case 4:
              // fancy_rihyan
              jsonds2 = await fancyRihyan(id);
              break;

            default:
              // default to fancy_live_api_virtual
              jsonds2 = await fancyLiveApiVirtual(id);
              break;
          }
        // } catch (err) {
        //   // handle/propagate errors from the API calls
        //   return res.json({
        //     status: false,
        //     message: err.message || "Fancy API error",
        //   });
        // }
        */
        let liverate = 0;
        ...
        jsonds2 = await fancyLiveApiVirtual(id);
        console.log("fancy api response:", jsonds2);

        if (jsonds2) {
          for (const record of jsonds2?.records || []) {
            const key = `${cat_mid}-${record?.SelectionId}.FY`.trim();
            const gameStatus = String(record?.GameStatus ?? "").trim();
            console.log("key:", key, " mid:", mid, " gameStatus:", gameStatus);
            if (key == mid && gameStatus == "") {
              api_response = JSON.stringify(record);
              t0 = "OPEN";
              if (typ.toUpperCase() === "B") {
                liverate = Number(record?.BackPrice1 ?? 0);
                liveprice = Number(record?.BackSize1 ?? 0);
              } else if (typ.toUpperCase() === "L") {
                liverate = Number(record?.LayPrice1 ?? 0);
                liveprice = Number(record?.LaySize1 ?? 0);
              }
              break;
            }
          }
        ...
        if (t0 == "OPEN") {
          ...
          if (ratok == 1) {
            ...
            if (pointok == 1) {
              ...
              bt_bets_data.master_sharing = master_sharing;
